        ...
        public static SettingsProvider CreateMyCustomSettingsProvider()
        {
            ...
            var provider = new SettingsProvider("Project/Water System (Boat Attack)", SettingsScope.Project)
            {
                ...
                activateHandler = (s, element) =>
                {
                    ...
                    var serializedQualitySettings = settings.FindProperty(nameof(WaterProjectSettings.qualitySettings));
                    for (int i = 0; i < qualitySettings.Length; i++)
                    {
                        if(serializedQualitySettings.arraySize > i)
                            qualitySettings[i] = serializedQualitySettings.GetArrayElementAtIndex(i);
                    }
